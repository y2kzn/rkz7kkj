...
export async function TournamentGetData(
  ...
  // Incomplete party after tournament has started → player shows as registered but cannot play.
  // BuildTeams intentionally skips any party whose length !== PartySize, so no match is created.
  const partyLen = Info.PartyMembers?.length ?? 0;
  if (partyLen !== Tour.PartySize && Info.PartyMembers) {
