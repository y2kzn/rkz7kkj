...
export async function GetTournamentList(MaxResults: number, Page: number, AccessToken: string) {
  ...
  // Prefer active / upcoming tournaments so Discord-created ones appear in-game immediately.
  // Exclude finished & canceled from the default feed; still paginate correctly.
  const ActiveFilter = {
    Status: {
      $nin: [TournamentStatus.Finished, TournamentStatus.Canceled],
    },
  };

  const TotalCount = await Tournament.countDocuments(ActiveFilter);
  const FoundTournaments = await Tournament.find(ActiveFilter)
    .sort({ StartTime: 1 }) // soonest first — newly created Discord tournaments show up right away
    .limit(MaxResults)
  ...
  for (let Tour of FoundTournaments) {
    ...
    // Don't send finished/canceled tournaments to the game client
